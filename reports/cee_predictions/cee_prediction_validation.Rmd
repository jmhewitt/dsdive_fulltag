...
```{r classification_accuracy, fig.cap='\\label{fig:classification_accuracy} Error rate for non-deep predictions based on classification threshold and number of observations used to make prediction.', warning=FALSE}
library(ggplot2)
library(ggthemes)
library(dplyr)
...
classification_accuracies = function(n_pred_obs, cutoff) {
  ...
  if(all('TRUE' %in% rownames(prediction.table),
         'TRUE' %in% colnames(prediction.table))) {
    true_pos = prediction.table['TRUE', 'TRUE']
  } else {
    true_pos = 0
  }
  
  if(all('FALSE' %in% rownames(prediction.table),
         'TRUE' %in% colnames(prediction.table))) {
    false_neg = prediction.table['FALSE', 'TRUE']
  } else {
    false_neg = 0
  }
  
  if(all('FALSE' %in% rownames(prediction.table),
         'FALSE' %in% colnames(prediction.table))) {
    true_neg = prediction.table['FALSE', 'FALSE']
  } else {
    true_neg = 0
  }
  ...
  if(all('TRUE' %in% rownames(prediction.table),
         'FALSE' %in% colnames(prediction.table))) {
    false_pos = prediction.table['TRUE', 'FALSE']
  } else {
    false_pos = 0
  }
...
ggplot(df, aes(x = n_pred_obs, y = false_omission_rate, col = factor(cutoff))) + 
  ...
  scale_x_continuous(breaks = unique(df$n_pred_obs)) + 
  scale_y_continuous(breaks = c(0,.05,.1,.2,.4,.6,.8)) +
...
```{r time_in_final_dive}
final_dive_time = do.call(rbind, lapply(1:nim_pkg$consts$n_subjects, 
                                    function(subj_ind) {
  # identify all segments associated with subject
  subj_segs = which(nim_pkg$consts$segments[,'subject_id'] == subj_ind)
  # indices of final segment
  final_seg_inds = seq(
    from = nim_pkg$consts$segments[tail(subj_segs, 1), 'start_ind'],
    to = nim_pkg$consts$segments[tail(subj_segs, 1), 'end_ind']
  )
  # use run length encoding to determine length of times on surface and at depth
  dive_periods = rle(nim_pkg$data$surface_bin[final_seg_inds])
  # final surface/depth state and time in state
  data.frame(
    subject = nim_pkg$consts$subject_id_labels[subj_ind],
    context = factor(
      x = tail(dive_periods$values, 1), 
      levels = c(FALSE, TRUE), 
      labels = c('Mid-dive', 'On surface')
    ),
    nobs = tail(dive_periods$lengths, 1)
  )
}))
...
knitr::kable(
  final_dive_time,
  caption = '\\label{tab:final_stage_times} Exposure context specifying whether animals were exposed during a dive or on the surface, as well as how many 1-minute observations are associated with the context.', 
  booktabs = TRUE
)
```
