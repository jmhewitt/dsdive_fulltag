...
non_intercept_covariates = grep(
  pattern = 'beta_tx\\[[^1].*', x = sampler_groups$beta_tx, value = TRUE
)
betas_hpds = HPDinterval(mcmc(param_samples[-burn, non_intercept_covariates]))

mean((betas_hpds[,'lower'] > 0) | (betas_hpds[, 'upper'] < 0) )
```
...
sub_groups = list(
  ...
  daytime = grep(
    pattern = 'beta_tx\\[2,.*', x = sampler_groups$beta_tx, value = TRUE
  ),
  moonlit = grep(
    pattern = 'beta_tx\\[3,.*', x = sampler_groups$beta_tx, value = TRUE
  ),
  prop_recent_deep = grep(
    pattern = 'beta_tx\\[4,.*', x = sampler_groups$beta_tx, value = TRUE
  ),
  prop_recent_deep = grep(
    pattern = 'beta_tx\\[5,.*', x = sampler_groups$beta_tx, value = TRUE
  )
)



sub_groups = lapply(1:nrow(nim_pkg$data$covariates), function(ind) { 
  grep(pattern = paste('beta_tx\\[', ind, ',.*', sep = ''), 
       x = sampler_groups$beta_tx, value = TRUE)
})
names(sub_groups) = rownames(nim_pkg$data$covariates)


  
targets::tar_load(movement_classes)
movement_classes$stage_defs

do.call(rbind, lapply(sub_groups, function(g) {
  hpds = HPDinterval(mcmc(param_samples[-burn, g]))
  
  data.frame(n_signif = sum((hpds[,'lower'] > 0) | (hpds[,'upper'] < 0)),
             n_total = nrow(hpds))
}))
