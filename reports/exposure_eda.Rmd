...
12 hours before an exposure, and depth bins observed in the first 12 hours 
after exposure.  
...
consecutive 12-hour sequences of unexposed depth bin observations.  The 
null distribution is approximated via sampling.

# Sampling units

Studying consecutive 12-hour windows is slightly different from both my initial 
...
12-hour time windows as the sampling unit, which is like a "time series" version 
...
When considering a time series approach, I chose to work with consecutive 
12-hour windows in order to highlight changes in short-term behavior, while 
using the Monte Carlo sampling to empirically average over different patterns of 
short-term behavior.  As Nic often mentions, short-term behvaior may vary 
accoring to longer-term behavior, which may be difficult to describe.
...
eda_stats = lapply(eda_data, function(tag_record) {
  res = bootstrap_exposure(
    depths = tag_record$depths, times = tag_record$times, window_length = 12, 
...
for(tagId in 1:length(eda_stats)) {
  ...
  if(is.finite(dat$exposure_time)) {
    date_range = dat$exposure_time + c(-1, 1) * hours(12)
    # base plot
    ...
    # highlight the pre/post 12 hour window
    pl = pl + 
      ...
      ggtitle('Tag data with pre/post exposure window outlined (black)')
    print(pl)
    cat(' \n\n')
  } else {
  ...
  if('p' %in% names(res)) {
    for(statInd in 1:length(res$p)) {
      ...
      pl = data.frame(x = res$null.samples[, statInd]) %>% 
        ...
        ggtitle(paste('p=', signif(res$p[statInd], 2), sep = '')) + 
        ylab('Density') + 
