...
eda_stats = lapply(eda_data, function(tag_record) {
  res = bootstrap_exposure(
    ...
    dive_labels = tag_record$labels, nsamples = 1e3, 
    ...
    statistic = function(x, y, x_labels = NULL, y_labels = NULL) {
      ...
      # marginal distribution of bins in shallow periods
      shallow_bin_count = table(
        df %>% 
          dplyr::filter(!(label %in% deep_labels)) %>% 
          dplyr::select(depth, type)
      )
      shallow_bin_dist = sweep(x = shallow_bin_count, MARGIN = 2, 
                               STATS = colSums(shallow_bin_count), FUN = '/')
      ...
      # count number of deep/shallow dives in x,y series
      df.counts = table(
        df %>% 
          dplyr::filter(label != 0) %>% 
          dplyr::group_by(label, type) %>% 
          dplyr::summarize(class = ifelse(max(depth) > 800, 
                                          'deep', 'shallow')) %>% 
      ...
      # extract raw data
      data = list(
        # marginal depth bin distributions
        marginal_bins = table(df %>% dplyr::select(depth, type)),
        # marginal depth bin distributions for shallow dives
        marginal_shallow_bins = shallow_bin_dist,
        # proportion of downward movements
        prop_downward = c(x = mean(dx.sign == 1), y = mean(dy.sign == 1)),
        # proportion of upward movements
        prop_upward = c(x = mean(dx.sign == -1), y = mean(dy.sign == -1)),
        # proportion of no-change "movements"
        prop_upward = c(x = mean(dx.sign == 0), y = mean(dy.sign == 0)),
        # total up distance
        total_upward = c(x = sum(dx[dx>0]), y = sum(dy[dy>0])),
        # total down distance
        total_downward = c(x = sum(dx[dx<0]), y = sum(dy[dy<0])),
        # total absolute distance
        total_movement = c(x = sum(abs(dx)), sum(abs(dy))),
        # proportion of surface observations in shallow periods
        prop_shallow_surface = shallow_summary$prop_surface,
        # total absolute distance in shallow periods
        total_shallow_movement = shallow_summary$total_distance,
        # number of deep/shallow dives
        dive_counts = df.counts
      )
      
      # compute sample statistics
      stats = c(
        # SSE between marginal depth bin distributions
        marginal_bins = sum(apply(table(df %>% dplyr::select(depth, type)), 
                                  1, diff)^2),
        # CvM between marginal depth bin distributions for shallow dives
        # marginal_shallow_bins = sum(apply(shallow_bin_dist, 1, diff)^2),
        marginal_shallow_bins = sum(
          diff(t(apply(shallow_bin_dist, 2, cumsum)))^2
        ),
        ...
        # SSE between number of deep/shallow dives
        dive_counts = sum(apply(df.counts, 1, function(x) diff(x)^2))
      )
...
stat_labels = c(
  'SSE between depth bin counts',
  'CvM between marginal depth bin distributions of shallow dives',
  'SSE between proportion of downward movements',
  'SSE between proportion of upward movements',
  'SSE between proportion of no-change "movements"',
  'SSE between total up distance',
...
```{r date_axis_minor_major, include = FALSE}
date_major_minor = function(x) {
  
  day_round = round_date(x, unit = 'day')
  hour_round = round_date(x, unit = 'hour')
  
  day_label = date(x)
  hour_label = hour(x)
  
  # labs = format.POSIXct(x, format = '%Hh')
  labs = x
  date_breaks = x == day_round
  date_breaks[is.na(date_breaks)] = FALSE
  labs[date_breaks] = format.POSIXct(x[date_breaks], format = '%d-%b')
...
for(tagId in 1:length(eda_stats)) {
  ...
  # print exposure time and other information
  cat('Exposure time: ', format(eda_data[[tagId]]$exposure_time, usetz = TRUE), 
      ' \n\n')
  cat('Pre-exposure data: ', round(as.numeric(res$pre_exposure_days), 1), 
      'days \n\n')
  cat('Total pre/lag/post window length: ', 
      round(as.numeric(res$analysis_duration_days), 1), 'days \n\n')
  
  cat('#### Data around exposure \n\n')
  
  # plot tag around exposure
  dat = eda_data[[tagId]]
  if(is.finite(dat$exposure_time)) {
    date_range = c(
      dat$exposure_time - hours(params$window_length),
      dat$exposure_time + hours(params$window_length + params$response_lag)
    )
    # base plot
    ...
    analysis_windows = data.frame(
      xmin = c(res$pre_exposed_window[1], res$exposed_window[1]),
      xmax = c(res$pre_exposed_window[2], res$exposed_window[2])
    )
    
    # highlight the pre/post window, and re-overlay exposure time
    pl = pl + 
      geom_rect(data = analysis_windows,
                mapping = aes(xmin = xmin, xmax = xmax,
                                 ymin = 0, ymax = Inf), inherit.aes = FALSE,
                   fill = NA, col = 'black', lwd = 2) +
      ggtitle(label = 'Tag data with pre/post windows outlined (black)', 
              subtitle = '(Exposure occurred at yellow vertical line)') + 
      geom_vline(xintercept = dat$exposure_time, lty = 1, col = 'yellow')
  ...
  cat('Results based on ', format(nrow(res$null.samples), big.mark = ','),
      ' Monte Carlo samples \n\n' )
  ...
  if('p' %in% names(res)) {
    ...
    cat('Summary statistic values for exposed pre/post: \n\n')
    
    cat('``` \n\n')
    
    print(res$test_data[[statInd]])
