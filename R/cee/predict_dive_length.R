predict_dive_length = function(
  post_output_dir, tag, burn, timestep, lon, lat, template_bins
) {
  ...
  # get index for last modeled stage observation
  ...
  # load samples for final stage
  stage_samples = do.call(c, sapply(stage_sample_files, function(f) {
    s = readRDS(f)
    s[,last_obs_ind]
  }))
  names(stage_samples) = NULL
  ...
  remaining_duration_preds = sapply((1:nrow(param_samples))[-burn], function(ind) {
    ...
    # extract stages from posterior predictive distribution
    sampled_stages = stage_samples[ind]
    # start forward simulation from final prediction for stage
    pred = fwd_sim_dive(
      stages = sampled_stages, depths = nim_pkg$data$depths[last_obs_ind], 
