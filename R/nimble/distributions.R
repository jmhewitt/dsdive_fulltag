...
ddiveShallow = nimbleFunction(
  run = function(x = double(1), times = double(1), expm = double(3),
    ...
    #   T - vector of dive-specific random effects;  c(T0, T1, T2) s.t.
    #       T0 is dive start time, T1 is the end of stage 1,
    #       and T2 is dive end time.  note that the indexing implies T[1] is
    #       start of dive, T[1] is start of stage 2,
    #       and T[2] is start of surface period (i.e., end of dive).
...
registerDistributions(list(
  ddiveShallow = list(
    BUGSdist =
    'ddiveShallow(times, expm, N, evecs, evals, d, dInv, tstep, include, M, T)',
    types = c('value = double(1)', 'times = double(1)', 'expm = double(3)',
              'N = integer(0)', 'evecs = double(3)', 'evals = double(2)',
              'd = double(2)', 'dInv = double(2)', 'tstep = double(0)',
              'include = logical(0)', 'M = integer(0)', 'T = double(1)')
